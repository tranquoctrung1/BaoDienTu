...
const kiemtrapassworddangnhap = document.getElementById(
  "kiemtrapassworddangnhap"
);
...
function KiemtraDangKy() {
  ...
  if (isNaN(parseInt(ngay.value))) {
    ...
    kiemtrapassworddangnhap.value == "" ||
    kiemtrapassworddangnhap.value == null
  ) {
    ...
    $.ajax({
      url: `/login/checkEmail?email=${email.value}`,
      type: "get",
      async: false,
      success: function (data) {
        if (data === false) {
